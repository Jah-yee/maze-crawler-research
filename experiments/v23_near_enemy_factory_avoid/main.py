"""
v23 Maze Crawler Agent: delay24 north mine economy with near enemy factory avoidance.
...
def _agent_inner(obs, config):
    ...
    visible_enemy_factory_near = (
        factory_gap > 3
        and any(abs(enemy[1] - factory_col) + abs(enemy[2] - factory_row) <= 4 for enemy in enemy_factories)
    )

    enemy_factory_threats = set()
    if collision_tiebreak_bad or visible_enemy_factory_near:
