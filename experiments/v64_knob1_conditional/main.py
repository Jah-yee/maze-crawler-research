"""
v64_knob1_conditional: v57 + conditional Knob1 (only block when gap > 3).

v57 unconditionally blocked enemy factory cells, which fixes replay 78598494
but costs tempo in normal play (z=-0.39 vs v37 at 80g).

v64: only block when factory_gap > 3 (not immediately dying).
When gap <= 3, collision is acceptable — a draw beats certain death.
This preserves the correctness fix while reducing the tempo penalty.
"""
...
def _agent_inner(obs, config):
    ...
    if collision_tiebreak_bad and factory_gap > 3:
        for enemy in enemy_factories:
