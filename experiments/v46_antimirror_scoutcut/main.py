"""
v46 Maze Crawler Agent: v44 + anti-mirror BFSDIRS + scout cap v37 + economy engine (early miner + TRANSFER回流 + mine idle).

Three improvements over v37:

1. EARLY MINER: when turn <= 25, relax the mine-build gate so we catch north
   mining nodes immediately from the start position. Also check E/W when the
   adjacent node is directly east/west (not just north) within the early window.
   Undirected BUILD_MINER if the factory itself is on a mining node.
   Top-15 get first miners at step 2-33; we were getting them at step 90-159
   or not at all (38% of games had 0 miners).

2. TRANSFER before TRANSFORM: when a miner is on a mining node and the factory
   is adjacent, do TRANSFER_<dir> to return ~200 energy to the factory BEFORE
   doing TRANSFORM. This cuts the net factory cost from 300 to ~100 and halves
   the break-even time from 6 turns to 2 turns. bunterrrrr's TRANSFER count is
   400+/game; ours was 0.

3. MINE IDLE gap 10 → 6: factory idles on its own mine (collecting +50/turn)
   when gap > 6 instead of gap > 10. A gap of 7+ rows is safe at any scroll
   speed; idling 4 extra gap-units of turns per mine compounds significantly.
"""
from collections import deque

FACTORY, SCOUT, WORKER, MINER = 0, 1, 2, 3
DIRS = ("NORTH", "EAST", "WEST", "SOUTH")
# Player-asymmetric direction orderings for anti-mirror tiebreak (D01/D02).
# p0 explores EAST before WEST; p1 explores WEST before EAST.
# This breaks the deterministic mirror that causes ~20% draws in v37 self-play
# with zero ELO cost on the ladder (both players are equally likely to be p0/p1).
...
SCOUT_DELAY_STEP = 24
EARLY_MINER_STEP = 25   # expanded mine-search window (any adjacent node, any dir)
...
def _agent_inner(obs, config):
    ...
    player = obs.player
    BFSDIRS = DIRS_P0 if player == 0 else DIRS_P1  # D01: anti-mirror
    ...
    if factory_uid not in actions:
        ...
        if f_build_cd <= 1 and counts[MINER] < 1:
            # Early window (turn <= EARLY_MINER_STEP): try N/E/W + undirected.
            # Gap is always huge early so skip the gap gate; lower energy floor.
            if turn <= EARLY_MINER_STEP and factory_energy >= 400:
                # Undirected: factory itself is on a mining node
                if (factory_col, factory_row) in mining_nodes:
                    mine_build_action = "BUILD_MINER"
                else:
                    # Directional: check N, E, W for adjacent node
                    for d in ("NORTH", "EAST", "WEST"):
                        dc, dr = OFFSETS[d]
                        mpos = (factory_col + dc, factory_row + dr)
                        if mpos in mining_nodes and mpos not in my_positions and can_move(factory_col, factory_row, d):
                            mine_build_action = f"BUILD_MINER_{d}"
                            break
            # Normal window: NORTH only, gap + energy gates
            if mine_build_action is None and factory_energy >= 650 and factory_gap > 6:
                for d in ("NORTH",):
                    dc, dr = OFFSETS[d]
                    mpos = (factory_col + dc, factory_row + dr)
        ...
        if mine_build_action:
            ...
            SCOUT_DELAY_STEP <= turn <= 32  # hard cap: 1 early scout only
            and spawn_ok
            and counts[SCOUT] < 1
            ...
            # Mine idle: gap > 6 (was > 10) — idle on own mine earlier for more +50/turn
            if (factory_col, factory_row) in own_mines and factory_gap > 6:
                record(factory_uid, "IDLE", factory_col, factory_row)
    ...
    # ========== MINER PLANNING ==========
    # TRANSFER before TRANSFORM: when the miner is on a mining node and the
    # factory is adjacent, first transfer energy back to the factory, then
    # transform next turn. This halves the effective mine-build cost (300→~100
    # net) because ~200 energy flows back immediately. bunterrrrr does 400+
    # TRANSFERs per game; we were doing 0.
    miners = [(uid, d) for uid, d in my_robots.items() if d[0] == MINER]
    for uid, data in miners:
        col, row = data[1], data[2]
        energy = data[3]
        if (col, row) in mining_nodes:
            # Find which direction the factory is relative to this miner
            factory_dir = None
            for d, (dc, dr) in OFFSETS.items():
                if (col + dc, row + dr) == (factory_col, factory_row):
                    factory_dir = d
                    break
            if factory_dir and energy > 150:
                # Transfer to factory first; miner retains ~100 for TRANSFORM next turn
                record(uid, f"TRANSFER_{factory_dir}", col, row)
            elif energy >= 100:
