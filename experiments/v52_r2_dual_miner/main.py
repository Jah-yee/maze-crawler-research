"""
v52_r2_dual_miner: v37 + W-Tree R2 (lift miner cap to 2 when adj north node).

Single-line change vs v37: the BUILD_MINER_NORTH gate at line 335 widens its
miner-count cap from `< 1` to `< 2` whenever (factory_col, factory_row + 1)
is still a mining node. Rationale: W-Tree L12 (precision 0.555, n=704) shows
top-15 factories build a second miner specifically when an adj-north mining
node is still available.

The mine_build_action loop already requires the target cell to be in
mining_nodes and not in my_positions, so the gate change only enables an
attempt; the loop still gates on geometry.
"""
...
def _agent_inner(obs, config):
    ...
    if factory_uid not in actions:
        ...
        miner_cap = 2 if (factory_col, factory_row + 1) in mining_nodes else 1
        if f_build_cd <= 1 and factory_energy >= 650 and factory_gap > 6 and counts[MINER] < miner_cap:
            for d in ("NORTH",):
                dc, dr = OFFSETS[d]
