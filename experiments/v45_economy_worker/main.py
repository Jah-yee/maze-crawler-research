"""
v45 Maze Crawler Agent: v44 + worker buffer v37 + economy engine (early miner + TRANSFER回流 + mine idle).
...
def _agent_inner(obs, config):
    ...
    if factory_uid not in actions:
        ...
        if mine_build_action:
            ...
            and turn >= 300
            and factory_gap >= 8
            and factory_energy >= 500
            and counts[WORKER] == 0
            and counts[MINER] >= 1
        ):
            # Late worker buffer: 1 worker as end-game energy/tiebreak buffer.
            # Only when we have mine economy running (counts[MINER]>=1).
