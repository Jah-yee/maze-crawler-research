"""
v42 Maze Crawler Agent: v37 + P01 enemy-current-cell fix + Final Kick boundary defense.

Built on v37 (online ~1142, strict improvement over v24: north_only_jump when
gap<=8 + scout/miner gap gates). Two narrow changes vs v37:

- P01 (correctness): the enemy factory's CURRENT cell is now treated as an
  occupied/unsafe cell UNCONDITIONALLY, not only when collision_tiebreak_bad.
  v40's regression (78598494) was walking NORTH into a stationary enemy factory
  because the tiebreak heuristic said "not at risk" so the cell was never added
  to threats. Ramming an enemy factory destroys our own factory (mutual
  destruction -> at best a draw), so its cell should never be a destination.

- Final Kick (boundary_scroll defense): at turn >= 400 the factory stops
  spending turns on BUILD_SCOUT / BUILD_MINER and stops camping its own mine;
  it races north and preserves energy for JUMP_NORTH escapes. v37 only gated
  builds by gap; this adds the step>=400 dimension that targets our 6 / Joseph's
  7 boundary_scroll deaths (high energy but scrolled off while building/camping).
"""
...
SCOUT_DELAY_STEP = 24
FINAL_KICK_STEP = 400  # late-game: stop building, race north
...
def _agent_inner(obs, config):
    ...
    # Final Kick: in the late game AND when the south gap is entering the danger
    # zone, stop spending factory turns on builds / mine-camping and race north
    # (boundary_scroll defense). If the gap is still comfortably large late, keep
    # compounding -- a step-480 game at gap=30 is safe and should not abandon its
    # economy. Scroll is fast past step 400, so 15 is a conservative danger line.
    final_kick = False  # isolation: P01-only (boundary disabled)
    ...
    # P01: the enemy factory's current cell is ALWAYS an unsafe destination
    # (ramming it destroys our own factory -> at best a draw). This must hold
    # regardless of the collision tiebreak heuristic; v40 died (78598494) by
    # walking into a stationary enemy because the heuristic gated this add.
    ...
    if factory_uid not in actions:
        ...
        if (not final_kick) and f_build_cd <= 1 and factory_energy >= 650 and factory_gap > 6 and counts[MINER] < 1:
            for d in ("NORTH",):
                dc, dr = OFFSETS[d]
        ...
        if mine_build_action:
            ...
            if (not final_kick) and (factory_col, factory_row) in own_mines and factory_gap > 10:
                record(factory_uid, "IDLE", factory_col, factory_row)
