"""
v50_combo = v50 container + v46c jump-cd preservation + v49 30% hash perturbation.

Components:
1. v50 utility-scoring architecture (proven parity vs v37, z=-0.09)
2. v46c: LOW_GAP_NORTH_ONLY widened 8->12, desperation jump tightened 3->2
3. v49: ~30% hash perturbation of lateral BFS order when factory_gap > 6

This is the first "container + validated patches" combination.
"""
import hashlib
...
_PERTURB_THRESHOLD = 77   # 77/256 ≈ 30.1%
...
LOW_GAP_NORTH_ONLY = 12     # v46c: widened from 8 to preserve jump cd longer
...
def _agent_inner(obs, config):
    ...
    # ------------------------------------------------------------------
    # ------------------------------------------------------------------
    # v49: anti-mirror hash-perturbed direction ordering
    # ------------------------------------------------------------------
    # Base player-asymmetric ordering (D01/D02)
    ...
    # Default bindings (overwritten after factory_gap is known)
    BFSDIRS = BFSDIRS_BASE
    LATERAL = LATERAL_BASE
    NEW_FALLBACK = ("NORTH",) + LATERAL
    DESPERATION_JUMP = LATERAL + ("SOUTH",)

    # D03: ~30% turns swap lateral order when safe (gap > 6)
    if factory_gap > 6:
        _h = hashlib.sha256(
            _HASH_SALT
    ...
    def score_factory_move(ctx):
        ...
        # Fallback 2: any northward walk (use perturbed lateral order).
        for d in NEW_FALLBACK:
            if ctx["can_move"](col, row, d) and ctx["safe_factory_action"](d):
                return (d, PRI_MOVE_ANY_NORTH)

        # Fallback 3: walk SOUTH to escape dead-end (only if gap <= 2, v46c).
        if gap <= EMERGENCY_GAP and ctx["can_move"](col, row, "SOUTH") and ctx["safe_factory_action"]("SOUTH"):
            return ("SOUTH", PRI_MOVE_SOUTH)

        # Fallback 4: desperation jump any direction (only if gap <= 2, v46c).
        if gap <= EMERGENCY_GAP and ctx["f_jump_cd"] <= 0:
