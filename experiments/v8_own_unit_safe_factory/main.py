...
def _agent_inner(obs, config):
    ...
    blocked_own_positions = {
        (d[1], d[2])
        for d in my_robots.values()
        if d[0] != FACTORY and (d[5] if len(d) > 5 else 0) > 1
    }
    reserved = set()
    ...
    def bfs_first_step(start, goals, depth=20, avoid_occupied=True, avoid_stuck_own=False):
        ...
        while q:
            ...
            for d in DIRS:
                ...
                if (nc, nr) in seen:
                    continue
                if avoid_stuck_own and (nc, nr) in blocked_own_positions:
                    continue
    ...
    def bfs_jump(start, goals, jump_cd, depth=20, avoid_stuck_own=False):
        """
    ...
    avoid_stuck_own = factory_gap > 3

    def factory_step_blocked_by_stuck_own(step):
        if not avoid_stuck_own or not step or step == "IDLE":
            return False
        if step.startswith("JUMP_"):
            d = step.split("_")[1]
            dc, dr = OFFSETS[d]
            dest = (factory_col + 2 * dc, factory_row + 2 * dr)
        elif step in OFFSETS:
            dc, dr = OFFSETS[step]
            dest = (factory_col + dc, factory_row + dr)
        else:
            return False
        return dest in blocked_own_positions
    ...
    if factory_uid not in actions:
        ...
        if spawn_ok and counts[SCOUT] < 1 and factory_energy >= 50:
            ...
            step = bfs_jump(
                ...
                f_jump_cd,
                depth=20,
                avoid_stuck_own=avoid_stuck_own,
            )
            ...
            if not step:
                for d in ("NORTH", "EAST", "WEST"):
                    if can_move(factory_col, factory_row, d) and not factory_step_blocked_by_stuck_own(d):
                        step = d
                        break
            ...
            if not step and f_jump_cd <= 0 and factory_gap <= 3:
                for d in ("EAST", "WEST", "SOUTH"):
                    act = f"JUMP_{d}"
                    if can_jump(factory_col, factory_row, d) and not factory_step_blocked_by_stuck_own(act):
                        step = act
                        break

            if step and step != "IDLE" and not factory_step_blocked_by_stuck_own(step):
