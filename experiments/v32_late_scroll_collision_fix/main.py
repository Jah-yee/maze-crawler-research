"""
v32 Maze Crawler Agent: v24 with late scroll and collision fixes.
Key ideas: jump-preferred BFS, mirror vision, optimistic fog, emergency escape,
an early no-scout window for bunterrrrr-style mine conversion, and tighter
late-game pressure handling around scroll and factory contact.
"""
...
def _agent_inner(obs, config):
    ...
    late_scroll_pressure = turn >= 350 or south >= 70
    urgent_scroll_gap = 2
    if late_scroll_pressure:
        urgent_scroll_gap = 6
    if south >= 95:
        urgent_scroll_gap = 10
    ...
    # Emergency: expand the safety margin in late game because scroll losses
    # often happen while the factory is still a few rows above the bound.
    ...
    if factory_uid not in actions:
        ...
        if mine_build_action:
            ...
            and not (late_scroll_pressure and factory_gap <= urgent_scroll_gap + 2)
        ):
            ...
            if (
                ...
                and
                (factory_col, factory_row) in own_mines
                and factory_gap > 10
                and not late_scroll_pressure
