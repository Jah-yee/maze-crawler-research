"""
v65: v37 + K6 Miner TRANSFER engine.

1. Lower first-miner threshold (energy >= 400, gap > 4)
2. Miner TRANSFER_SOUTH before TRANSFORM (send energy back to factory)
3. Factory TRANSFER_NORTH to miner above (complete the K6 cycle)
4. Factory preferred mine harvesting (NORTH onto own mine)

Top-15 teams average first miner at step 27 vs our ~90+.
The K6 "triple kill" is the core economy engine of all top-5 teams.
"""
...
def _agent_inner(obs, config):
    ...
    if factory_uid not in actions:
        ...
        if f_build_cd <= 1 and factory_energy >= 400 and factory_gap > 4 and counts[MINER] < 1:
            for d in ("NORTH",):
                dc, dr = OFFSETS[d]
            ...
            if factory_uid in actions:
                ...
                for d in ("NORTH",):
                    ...
                    if mpos in own_mines and can_move(factory_col, factory_row, d) and safe_factory_action(d):
                        ...
                        break
                # K6: TRANSFER_NORTH to miner on mining node above
                if factory_uid not in actions and factory_energy > 500:
                    for uid, d in my_robots.items():
                        if (d[0] == MINER and d[1] == factory_col and d[2] == factory_row + 1
                                and d[3] >= 100 and d[3] < 200 and (d[1], d[2]) in mining_nodes):
                            record(factory_uid, "TRANSFER_NORTH", factory_col, factory_row)
                            break
            if factory_uid in actions:
                pass
            else:
    ...
    # ========== MINER PLANNING ==========
    # K6 engine: TRANSFER_SOUTH energy back to factory before TRANSFORM
    miners = [(uid, d) for uid, d in my_robots.items() if d[0] == MINER]
    for uid, data in miners:
        col, row = data[1], data[2]
        energy = data[3]
        on_node = (col, row) in mining_nodes
        if on_node and energy >= 200:
            if col == factory_col and row == factory_row + 1:
                record(uid, "TRANSFER_SOUTH", col, row)
            else:
                record(uid, "TRANSFORM", col, row)
        elif on_node and energy >= 100:
            record(uid, "TRANSFORM", col, row)
        else:
            record(uid, "IDLE", col, row)
