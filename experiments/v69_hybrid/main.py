"""
v63: v37 + dynamic scout energy + side mine creation (idle-only).

v37 base preserves the two boundary_scroll fixes.
- Dynamic scout energy: 50/150/250 based on turn phase
- Side mine creation: when no NORTH mine and f_move_cd > 1, try EAST/WEST
"""
...
SCOUT_DELAY_STEP = 24
SIDE_MINE_MIN_ENERGY = 1200
SIDE_MINE_MIN_GAP = 14
...
def _agent_inner(obs, config):
    ...
    # Ramming an enemy factory = mutual destruction = at best a draw.
    ...
    if factory_uid not in actions:
        ...
        if f_build_cd <= 1 and factory_energy >= 650 and factory_gap > 6 and counts[MINER] < 1:
            for d in ("NORTH",):
                ...
                break
            if (
                mine_build_action is None
                and f_move_cd > 1
                and factory_energy >= SIDE_MINE_MIN_ENERGY
                and factory_gap > SIDE_MINE_MIN_GAP
            ):
                for d in ("EAST", "WEST"):
                    dc, dr = OFFSETS[d]
                    mpos = (factory_col + dc, factory_row + dr)
        ...
        # R1 TRANSFER_NORTH: donate energy to north-adjacent unit when
        # factory is move-ready, no miner exists yet, and adjacent unit
        # has 280-600 energy (learned band from top-15 replays).
        ...
        scout_min_energy = 50
        if turn >= 300:
            scout_min_energy = 150
        if turn >= 400:
            scout_min_energy = 250
        if mine_build_action:
            ...
            and spawn_ok
            and counts[SCOUT] < 1
            and factory_energy >= scout_min_energy
            and factory_gap > 4
        ):
