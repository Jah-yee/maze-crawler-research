"""
v120: v67 + terminal safety precommit.

This tests the cleaner version of v112's idea: when terminal risk is high and
the factory would otherwise spend or idle, commit a safe north-progress factory
action inside the factory planning flow so later support planning sees the
reserved destination. No support actions, economy gates, or hash pathing are
changed.
"""
import hashlib
...
def _agent_inner(obs, config):
    ...
    def terminal_safety_replacement():
        if not (turn >= 400 or factory_gap <= 4):
            return None
        if (
            f_jump_cd <= 0
    ...
    if factory_uid not in actions:
        ...
        terminal_precommit = terminal_safety_replacement()
        if terminal_precommit and (mine_build_action or scout_build_candidate or f_move_cd > 1):
            record(factory_uid, terminal_precommit, factory_col, factory_row)
        elif mine_build_action:
            record(factory_uid, mine_build_action, factory_col, factory_row)
            counts[MINER] += 1
        elif scout_build_candidate:
            ...
            if factory_uid in actions:
                ...
                if not step or step in {"IDLE", "SOUTH", "JUMP_SOUTH"}:
                    replacement = terminal_safety_replacement()
                    if replacement:
                        step = replacement
