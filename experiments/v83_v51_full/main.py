...
def _agent_inner(obs, config):
    ...
    final_kick = turn >= 400 and factory_gap <= 15
    ...
    if factory_uid not in actions:
        ...
        if (not final_kick) and f_build_cd <= 1 and counts[MINER] < 1:
            dc, dr = OFFSETS["NORTH"]
            ...
            if mpos in mining_nodes and mpos not in my_positions and can_move(factory_col, factory_row, "NORTH"):
                if turn <= 25 and factory_energy >= 400:
                    mine_build_action = "BUILD_MINER_NORTH"  # early window
                elif factory_energy >= 650 and factory_gap > 5:
                    mine_build_action = "BUILD_MINER_NORTH"  # normal window
