...
def _agent_inner(obs, config):
            ...
            if factory_uid in actions:
                ...
                for d in ("NORTH",):
                    ...
                    if mpos in own_mines and can_move(factory_col, factory_row, d) and safe_factory_action(d):
                        ...
                        break
                # K6: TRANSFER_NORTH to miner above that needs energy for TRANSFORM
                if factory_uid not in actions and f_move_cd > 1 and factory_energy > 600:
                    tc, tr = factory_col, factory_row + 1
                    for tuid, td in my_robots.items():
                        if (td[0] == MINER and td[1] == tc and td[2] == tr
                                and td[3] < 100 and td[3] > 0
                                and (tc, tr) in mining_nodes):
    ...
    for uid, data in miners:
        ...
        if (col, row) in mining_nodes:
            if col == factory_col and row == factory_row + 1 and energy >= 100:
