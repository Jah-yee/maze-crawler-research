"""
v36 Maze Crawler Agent: v24 + save factory jump for emergencies in late game.

Targets the largest current online failure mode for v24 (boundary_scroll
losses): when the factory keeps using JUMP for fast travel, the 20-turn
cooldown frequently expires AT a future low-gap moment and we cannot escape.

Single-knob change vs v24: in late game (south >= 60), only allow
JUMP_NORTH expansion in the factory BFS when there is real lateral need.
JUMP_EAST / JUMP_WEST / JUMP_SOUTH still occur via emergency / fallback
branches when survival demands them, but the planner prefers walking
laterally to keep the jump cooldown ready for a NORTH escape.
"""
...
def _agent_inner(obs, config):
    ...
    def bfs_jump(start, goals, jump_cd, depth=20, north_only_jump=False):
        ...
        when scroll pressure makes lateral jumps risky.
        """
        if not goals:
            return None
        goal_set = set(goals)
    ...
    if factory_uid not in actions:
        ...
        if mine_build_action:
            ...
            if factory_uid in actions:
                ...
                # In late game (south >= 60) we keep the jump cooldown for a
                # NORTH escape rather than spending it on lateral travel; the
                # emergency block above and explicit fallbacks still allow
                # lateral / SOUTH jumps when the factory is genuinely stuck.
                target_row = min(north, factory_row + 20)
                factory_goals = [(tc, target_row) for tc in range(width)]
                save_jump_for_north = south >= 60 and factory_gap > 2
                step = bfs_jump(
                    (factory_col, factory_row),
                    factory_goals,
                    f_jump_cd,
                    depth=20,
                    north_only_jump=save_jump_for_north,
                )
