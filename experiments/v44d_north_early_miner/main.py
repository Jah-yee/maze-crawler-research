"""
v44d: early NORTH miner (turn<=25 energy>=400) + TRANSFER + idle gap>10 v37 + economy engine (early miner + TRANSFER回流 + mine idle).
...
def _agent_inner(obs, config):
    ...
    if factory_uid not in actions:
        ...
        if f_build_cd <= 1 and counts[MINER] < 1:
            # Early window (turn <= EARLY_MINER_STEP): NORTH only with relaxed gates.
            # E/W early miners are pointless — factory can only collect from a NORTH
            # mine (the step-onto-mine branch only checks NORTH). Lower energy floor
            # since gap is always huge early and scroll hasn't started.
            if turn <= EARLY_MINER_STEP and factory_energy >= 400:
                dc, dr = OFFSETS["NORTH"]
                mpos = (factory_col + dc, factory_row + dr)
                if mpos in mining_nodes and mpos not in my_positions and can_move(factory_col, factory_row, "NORTH"):
                    mine_build_action = "BUILD_MINER_NORTH"
            # Normal window: NORTH only, standard gap + energy gates
        ...
        if mine_build_action:
            ...
            # Mine idle: gap > 10 (conservative — don't sacrifice row for energy)
