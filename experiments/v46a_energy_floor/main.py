"""
v46a = v37 + energy hard floor only.

Defense isolation: enable BUILD_* only when
factory_energy >= max((500 - turn) * 1.5, 500).
Everything else is byte-identical to v37.
"""
...
def _agent_inner(obs, config):
    ...
    # D1 energy hard floor: forbid BUILD_* below the floor.
    ...
    if factory_uid not in actions:
        ...
        if (not low_energy_lock) and f_build_cd <= 1 and factory_energy >= 650 and factory_gap > 6 and counts[MINER] < 1:
            for d in ("NORTH",):
                dc, dr = OFFSETS[d]
        ...
        if mine_build_action:
            ...
            if factory_uid in actions:
                ...
                if step and step != "IDLE":
                    ...
                    if (not low_energy_lock) and spawn_ok and counts[WORKER] < 1 and factory_energy >= 200 and factory_gap <= 4:
