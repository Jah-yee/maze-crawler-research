"""
v45 Maze Crawler Agent: v37 + D01 anti-mirror tiebreak (Step 4 of cycle).

v43 (late worker) and v44 (early miner) both failed paired_eval vs v37
(z=-0.24 and z=-0.87), so the worker/economy hypotheses are off the table.
Step 4 falls back to D01: when this agent is on player == 1, lateral
tiebreaks default to WEST before EAST. Many top opponents are presumed to
mirror v37-like east-first tiebreaks, so two such agents head into the
same lateral cell and one loses to a draw / collision tiebreak. Swapping
our player-1 order to west-first breaks the symmetric tie without
changing any tactical logic.

One narrow change vs v37: at the top of _agent_inner we bind four local
tie-order tuples (dirs_order, lateral, north_lateral, desperation) based
on obs.player, and the BFS / fallback / desperation loops use those
instead of the hardcoded `DIRS` / ("EAST", "WEST") / ("NORTH", "EAST",
"WEST") / ("EAST", "WEST", "SOUTH") tuples. Enemy-threat enumeration is
NOT rebound — those are set-additions where order does not matter.
"""
...
def _agent_inner(obs, config):
    ...
    # D01 anti-mirror: on player==1, prefer WEST before EAST in every lateral
    # tiebreak. Doesn't change which moves are *legal*, only which one wins
    # when several are equally good — enough to break symmetric collisions
    # against mirror opponents.
    if player == 1:
        dirs_order = ("NORTH", "WEST", "EAST", "SOUTH")
        lateral = ("WEST", "EAST")
        north_lateral = ("NORTH", "WEST", "EAST")
        desperation = ("WEST", "EAST", "SOUTH")
    else:
        dirs_order = DIRS
        lateral = ("EAST", "WEST")
        north_lateral = ("NORTH", "EAST", "WEST")
        desperation = ("EAST", "WEST", "SOUTH")
    ...
    def bfs_jump(start, goals, jump_cd, depth=20, north_only_jump=False):
        ...
        jump_dirs = ("NORTH",) if north_only_jump else dirs_order
        while q:
