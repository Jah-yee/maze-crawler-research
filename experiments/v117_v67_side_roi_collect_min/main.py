"""
v117: v67 + narrow side-mine ROI gate.

Keep v67's NORTH miner rule untouched. If no miner exists and a side mining
node is route-compatible, allow exactly one EAST/WEST first miner only when
the factory would otherwise spend a move-cooldown turn idling.
"""
import hashlib
...
def _agent_inner(obs, config):
    ...
    def bfs_distance(start, goals, depth=12, extra_start=None):
        if not goals:
            return None
        goal_set = set(goals)
        if start in goal_set:
        ...
        while q:
            ...
            for d in bfs_dirs:
                ...
                if pos in reserved:
                    continue
                if pos in my_positions and pos != start and pos != extra_start and pos not in goal_set:
                    continue
                if pos in goal_set:
                    return dist + 1
                seen.add(pos)
    ...
    if factory_uid not in actions:
        ...
        if f_build_cd <= 1 and factory_energy >= 650 and factory_gap > 6 and counts[MINER] < 1:
            for d in ("NORTH",):
                ...
                break
            if mine_build_action is None and f_move_cd > 1 and factory_gap > 12 and turn <= 380:
                target_row = min(north, factory_row + 5)
                goals = [(tc, target_row) for tc in range(width)]
                base_dist = bfs_distance((factory_col, factory_row), goals, depth=12)
                for d in lateral_order:
                    dc, dr = OFFSETS[d]
                    mpos = (factory_col + dc, factory_row + dr)
                    if mpos not in mining_nodes or mpos in my_positions:
                        continue
                    if not can_move(factory_col, factory_row, d):
                        continue
                    after_dist = bfs_distance(mpos, goals, depth=12, extra_start=mpos)
                    if base_dist is None or after_dist is None:
                        continue
                    if 1 + after_dist - base_dist > 1:
                        continue
                    mine_build_action = f"BUILD_MINER_{d}"
                    break
