"""
v40 Maze Crawler Agent: v37 + three knobs targeting factory-collision losses.

v37 online sample (20 public episodes) showed 5/7 losses are factory_collision,
4 of which are real (excl. validation):
  - 3/4 "rammed on cooldown": our mvcd>1, enemy walks into our cell.
  - 1/4 mutual destruction: both factories step into the same cell.
  - 2/4 of those losses we had ZERO support units, a direct side-effect of
    v37's `factory_gap > 4` BUILD_SCOUT gate.

Three narrow changes vs v37:

Knob 1 - 2-turn collision predictor.
  When `collision_tiebreak_bad` is true, also block cells reachable by a
  visible enemy factory within 2 of its move slots (`emove_cd <= 2`),
  because our factoryMovePeriod=2 cooldown keeps us stuck during exactly
  the same window the opponent uses to ram us.

Knob 2 - Conditional BUILD_SCOUT.
  Keep v37's `factory_gap > 4` rule, but ALSO allow BUILD_SCOUT when no
  visible enemy factory sits within Manhattan 6 of us. A 50-energy scout
  is cheap tiebreak insurance when no opponent is close enough to punish
  the build turn.

Knob 3 - BUILD_WORKER energy floor.
  Require factory_energy >= 350 (was >= 200). Worker spawn drains
  200 + 200 = 400, which is exactly what causes v24/v38's repeated
  energy=0 forced-IDLE JUMP_NORTH boundary_scroll losses.
"""
...
def _agent_inner(obs, config):
    ...
    if collision_tiebreak_bad:
        for enemy in enemy_factories:
            ...
            # Knob 1: include 2-turn enemy reach when tiebreak is bad.
            # Our factoryMovePeriod is 2, so once we move we are stuck for
            # 2 turns; the enemy gets one move slot in that window even if
            # they are currently on cooldown (emove_cd <= 2 becomes 0 in
            # time to ram our locked cell).
            if emove_cd <= 2:
                for d in DIRS:
                    if can_move(ec, er, d):
    ...
    if factory_uid not in actions:
        ...
        # Knob 2: BUILD_SCOUT in two situations only.
        #  (a) we are safely out of the danger window (factory_gap > 4),
        #  (b) OR no visible enemy factory is within Manhattan 6, so the
        #      scout build turn is not exposing us to a contact ram. A
        #      50-energy scout still buys tiebreak insurance for later.
        scout_safe_distance = all(
            abs(enemy[1] - factory_col) + abs(enemy[2] - factory_row) > 6
            for enemy in enemy_factories
        )
        scout_build_ok = (
            ...
            and (factory_gap > 4 or scout_safe_distance)
        )
        if mine_build_action:
            ...
            if factory_uid in actions:
                ...
                if step and step != "IDLE":
                    ...
                    # Knob 3: worker spawn drains 200 (cost) + 200 (spawn
                    # energy) = 400 from the factory; with energyPerTurn=1
                    # the difference between 200 and 350 is the gap between
                    # "factory hits zero before scroll catches up" and
                    # "factory keeps enough fuel for a JUMP_NORTH escape".
                    if spawn_ok and counts[WORKER] < 1 and factory_energy >= 350 and factory_gap <= 4:
