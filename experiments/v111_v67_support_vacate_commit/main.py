"""
v111: v67 + narrow support-vacate commit.

Keeps v67 hash/path perturbation and economy unchanged. Adds a small final
postprocess: when the factory is under pressure and its chosen route target is
occupied by our own support, try to move that support aside safely.
"""
import hashlib
...
def _agent_inner(obs, config):
    ...
    # PIBT-lite postprocess: let a support unit vacate the factory's committed
    # route cell only under pressure, without changing build/economy choices.
    f_action = actions.get(factory_uid, "IDLE")
    f_dest = action_dest(factory_col, factory_row, f_action)
    blocker_uid = my_positions.get(f_dest)
    if (
        blocker_uid
        and blocker_uid != factory_uid
        and f_action != "IDLE"
        and not f_action.startswith("BUILD_")
        and factory_gap <= 8
    ):
        blocker = my_robots.get(blocker_uid)
        if blocker and blocker[0] != FACTORY:
            b_col, b_row = blocker[1], blocker[2]
            b_move_cd = blocker[5] if len(blocker) > 5 else 0
            if b_move_cd <= 1:
                occupied = set(my_positions)
                occupied.discard((b_col, b_row))
                occupied.discard((factory_col, factory_row))
                occupied.update(pos for pos in reserved if pos != f_dest)
                for d in ("EAST", "WEST", "NORTH", "SOUTH"):
                    if not can_move(b_col, b_row, d):
                        continue
                    nc, nr = b_col + OFFSETS[d][0], b_row + OFFSETS[d][1]
                    if (nc, nr) == (factory_col, factory_row):
                        continue
                    if (nc, nr) in occupied:
                        continue
                    actions[blocker_uid] = d
                    reserved.add((nc, nr))
                    break
