"""
v27 Maze Crawler Agent: v24 plus a cautious enemy factory contact guard.
Key ideas: jump-preferred BFS, mirror vision, optimistic fog, emergency escape,
an early no-scout window for bunterrrrr-style mine conversion, and avoiding
voluntary contact with much stronger visible enemy factories.
"""
...
def _agent_inner(obs, config):
    ...
    def bad_factory_contact_after_move(dest):
        if factory_gap <= 4:
            return False
        for enemy in enemy_factories:
            ec, er = enemy[1], enemy[2]
            if abs(dest[0] - ec) + abs(dest[1] - er) != 1:
                continue
            emove_cd = enemy[5] if len(enemy) > 5 else 0
            enemy_energy = enemy[3]
            enemy_stronger = enemy_energy > factory_energy + 100
            support_not_better = own_support_energy <= visible_enemy_support_energy + 30
            if enemy_stronger and support_not_better and emove_cd <= 2:
                return True
        return False
    ...
    def safe_factory_action(act):
        ...
        if act.startswith("JUMP_") and dest in enemy_factory_jump_threats:
            return False
        if act in OFFSETS and bad_factory_contact_after_move(dest):
            return False
        return True
