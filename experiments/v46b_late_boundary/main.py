"""
v46b = v37 + late-game boundary defense only (improved over v42b).

When turn >= 400 AND factory_gap <= 10:
- ban SOUTH walk, JUMP_SOUTH, JUMP_EAST, JUMP_WEST
- ban all BUILD_* and own-mine camping
- ALLOW lateral walks (EAST / WEST) so we can still escape sideways. v42b
  banned them too which over-fit; the user's reading was that single-issue
  v42b went z=-0.24 partly because it ALSO did nothing (final_kick variable
  was declared but never gated anything). Here we wire it through.
"""
...
def _agent_inner(obs, config):
    ...
    # D2 late-game boundary defense.
    late_boundary = turn >= 400 and factory_gap <= 10
    ...
    def safe_factory_action(act, allow_emergency_jump=False):
        ...
        if act.startswith("JUMP_") and dest in enemy_factory_jump_threats:
            return False
        # D2: in late-game boundary mode, never go SOUTH and don't burn jump cd
        # on lateral. Lateral *walks* (EAST/WEST) are still allowed. The
        # emergency block (gap<=2) bypasses the lateral-jump ban via
        # allow_emergency_jump because there scroll-death is worse than jcd lock.
        if late_boundary and act in ("SOUTH", "JUMP_SOUTH"):
            return False
        if late_boundary and not allow_emergency_jump and act in ("JUMP_EAST", "JUMP_WEST"):
            return False
        return True
    ...
    if factory_gap <= 2 and south > 0:
        if f_jump_cd <= 0 and can_jump(factory_col, factory_row, "NORTH") and safe_factory_action("JUMP_NORTH"):
            ...
            for d in ("EAST", "WEST"):
                act = f"JUMP_{d}"
                if can_jump(factory_col, factory_row, d) and safe_factory_action(act, allow_emergency_jump=True):
                    dc, dr = OFFSETS[d]
    ...
    if factory_uid not in actions:
        ...
        if (not late_boundary) and f_build_cd <= 1 and factory_energy >= 650 and factory_gap > 6 and counts[MINER] < 1:
            for d in ("NORTH",):
                dc, dr = OFFSETS[d]
        ...
        if mine_build_action:
            ...
            if factory_uid in actions:
                ...
                # Fallback 4: desperation jump any direction (gap<=3 emergency
                # bypasses D2's lateral-jump ban; scroll-death is worse).
                ...
                if step and step != "IDLE":
                    ...
                    if (not late_boundary) and spawn_ok and counts[WORKER] < 1 and factory_energy >= 200 and factory_gap <= 4:
