"""
verify_a16: v37 + W6 A16 early-miner (turn <= 10) probe.

Minimal diff from v37: the BUILD_MINER_NORTH gate is widened when turn <= 10
to use a 300 energy floor and gap > 4 instead of the default 650/gap>6. Outside
the early window v37 behavior is identical.
...
def _agent_inner(obs, config):
    ...
    if factory_uid not in actions:
        ...
        early_miner_window = turn <= 10
        miner_energy_floor = 300 if early_miner_window else 650
        miner_gap_floor = 4 if early_miner_window else 6
        if f_build_cd <= 1 and factory_energy >= miner_energy_floor and factory_gap > miner_gap_floor and counts[MINER] < 1:
            for d in ("NORTH",):
                dc, dr = OFFSETS[d]
