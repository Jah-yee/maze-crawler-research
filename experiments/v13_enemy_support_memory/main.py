...
def _agent_inner(obs, config):
    ...
    step_no = getattr(obs, "step", 0) or 0
    ...
    if step_no <= 1 or "enemy_support_seen" not in _memory:
        _memory["enemy_support_seen"] = {}
    enemy_support_seen = _memory["enemy_support_seen"]
    for uid, d in enemy_robots.items():
        if d[0] != FACTORY:
            enemy_support_seen[uid] = (int(d[3]), step_no)

    estimated_enemy_support_energy = visible_enemy_support_energy
    estimated_enemy_support_count = visible_enemy_support_count
    remembered_energy = 0
    remembered_count = 0
    for energy, last_seen in list(enemy_support_seen.values()):
        age = max(0, step_no - last_seen)
        estimate = max(0, int(energy) - age // 4)
        if estimate > 0:
            remembered_energy += estimate
            remembered_count += 1
    if remembered_count > 0:
        estimated_enemy_support_energy = max(estimated_enemy_support_energy, remembered_energy)
        estimated_enemy_support_count = max(estimated_enemy_support_count, remembered_count)

    collision_tiebreak_bad = (
        factory_gap > 3
        and estimated_enemy_support_count > 0
        and (
            own_support_energy + 15 < estimated_enemy_support_energy
            or (
                own_support_energy <= estimated_enemy_support_energy + 15
                and own_support_count < estimated_enemy_support_count
            )
        )
    )
    ...
    def safe_factory_action(act):
        if not enemy_factory_threats:
            return True
        return action_dest(factory_col, factory_row, act) not in enemy_factory_threats
    ...
    if factory_uid not in actions:
        ...
        if spawn_ok and counts[SCOUT] < 1 and factory_energy >= 50:
