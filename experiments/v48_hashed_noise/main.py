"""
v48 Maze Crawler Agent: v47 + ~5% hash-based lateral perturbation (D03).

On top of v47's player-asymmetric DIRS, ~5% of turns swap the EAST/WEST
order for that turn's tie-break only. The trigger is a deterministic salted
hash of (turn, factory_col, factory_row, player), so different seeds /
games produce different perturbation schedules but the same seed is
reproducible.

Gated to skip emergency / mine-econ-critical turns:
- factory_gap > 4 (no swap when near death)
- factory_uid still owns a mine adjacent: not gated explicitly, but the
  mine-walk / mine-idle code reads LATERAL only through fallback, so a
  swap there is harmless.

Everything else is identical to v47.
"""
import hashlib
from collections import deque

_HASH_SALT = b"\x9a\x7d\xc4\x33\x88\x21\x55\x77"
_PERTURB_THRESHOLD = 13   # 13/256 ≈ 5.1% of turns swap lateral order
