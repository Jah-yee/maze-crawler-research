"""
v44 Maze Crawler Agent: v37 + early miner (Step 2 of v43-v45 cycle).

v37 only ever builds a miner if (energy >= 650, gap > 6, N-adjacent node,
miner_count < 1). On most maps that gate first opens around step 90-160,
and a substantial fraction of games finish with 0 miners. Top-15 agents
(bunterrrrr et al.) place their first miner at step 2-33 by relaxing the
energy / gap / direction gates very early.

One narrow change vs v37: an early-miner window. When
turn <= EARLY_MINER_STEP (25) AND counts[MINER] == 0 AND
factory_energy >= 500 AND any of (NORTH, EAST, WEST) neighbours is a
mining node we can walk into, emit BUILD_MINER_<dir>. The normal
energy-650 / gap>6 / NORTH-only mine block stays after this and runs on
turns 26+. No TRANSFER changes (Step 3 = v44b). No scout / save-jump
changes (preserve v37 boundary_scroll fixes).
"""
...
def _agent_inner(obs, config):
    ...
    if factory_uid not in actions:
        ...
        # Early-miner window: relax gap + direction gates so we catch a
        # NORTH/EAST/WEST adjacent mining node in the first ~25 turns when
        # gap is always huge and the factory hasn't accumulated 650 energy yet.
        if (
            f_build_cd <= 1
            and turn <= EARLY_MINER_STEP
            and counts[MINER] < 1
            ...
            for d in ("NORTH", "EAST", "WEST"):
                ...
                break
        # Normal-window NORTH-only mine build (v37 behaviour, post-turn-25).
        if mine_build_action is None and f_build_cd <= 1 and factory_energy >= 650 and factory_gap > 6 and counts[MINER] < 1:
            for d in ("NORTH",):
                dc, dr = OFFSETS[d]
                mpos = (factory_col + dc, factory_row + dr)
                if mpos not in mining_nodes or mpos in my_positions:
