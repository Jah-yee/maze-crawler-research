"""
v46cd_combined = v37 + D3 jump-cd preservation + D4 gap==3 emergency expansion.

D3: BFS north_only_jump widened gap<=8 -> gap<=12; fallback-4 lateral jump
    tightened gap<=3 -> gap<=2.
D4: at gap==3 AND f_jump_cd<=0 AND can_jump(NORTH), preemptively JUMP_NORTH.

Defense bundle on the two micro-changes that did not lose individually
(v46c, v46d both z=+0.37 over 80 paired games vs v37); D1 (energy floor)
and D2 (late boundary) were dropped because they showed z<=-1.57 and
z<=-2.14 in isolation.
"""
...
def _agent_inner(obs, config):
    ...
    # D4: preemptive escape at gap==3 (NORTH jump) before the gap<=2 block.
    if (
        factory_gap == 3
        and south > 0
        and f_jump_cd <= 0
    ...
    if factory_uid not in actions:
        ...
        if mine_build_action:
            ...
            if factory_uid in actions:
                ...
                # Fallback 4: desperation jump any direction (tightened to
                # gap<=2 so we almost never burn jcd on a lateral)
