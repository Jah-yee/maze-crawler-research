...
def _agent_inner(obs, config):
    ...
    if factory_uid not in actions:
        ...
        _enemy_near = any(abs(ef[1] - factory_col) + abs(ef[2] - factory_row) <= 5 for ef in enemy_factories)
        _miner_energy = 450 if _enemy_near else 350
        if f_build_cd <= 1 and factory_energy >= _miner_energy and factory_gap > 3 and counts[MINER] < 1:
            for d in ("NORTH",):
                dc, dr = OFFSETS[d]
