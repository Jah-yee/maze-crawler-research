"""
v46c_plus_v44b = v46c (jump-cd preservation, owned by defense line)
              + v44b (TRANSFER before TRANSFORM, owned by economy line).

Stack experiment only — used to confirm the two modifications compose
cleanly (factory_planning vs MINER planning, disjoint code paths) and
that the combined z is non-negative. The final code on each line is
owned by the respective worker; this file is for stack validation.
"""
...
def _agent_inner(obs, config):
    ...
    # ========== MINER PLANNING ==========
    # v44b TRANSFER before TRANSFORM: halves net mine-build cost (300->~100).
