"""
verify_w2: v37 + W2 energy hard floor probe.

Minimal diff from v37: when factory_energy is below
    max((500 - turn) * 1.5, 500)
all BUILD_MINER/SCOUT/WORKER branches in factory planning are disabled
(forcing IDLE/move/jump fallbacks). Spirit of W2: never overspend when the
factory cannot afford a build round.
...
def _agent_inner(obs, config):
    ...
    if factory_uid not in actions:
        ...
        # W2 energy hard floor: skip BUILD_* when factory cannot truly afford it.
        energy_floor = max((500 - turn) * 1.5, 500)
        energy_starved = factory_energy < energy_floor
        if not energy_starved and f_build_cd <= 1 and factory_energy >= 650 and factory_gap > 6 and counts[MINER] < 1:
            for d in ("NORTH",):
                dc, dr = OFFSETS[d]
        ...
        if mine_build_action:
            ...
            if factory_uid in actions:
                ...
                if step and step != "IDLE":
                    ...
                    if not energy_starved and spawn_ok and counts[WORKER] < 1 and factory_energy >= 200 and factory_gap <= 4:
