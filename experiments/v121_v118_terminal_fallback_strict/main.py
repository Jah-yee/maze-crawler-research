"""
v121: v118 + strict terminal fallback.

Keep v118's economy/support shape unchanged. Add only a narrow final-kick
fallback when low gap and persistent route-blocked state leave the factory with
a weak terminal route action.
"""
import hashlib
...
def _agent_inner(obs, config):
    ...
    if factory_uid not in actions:
        ...
        if mine_build_action:
            ...
            and f_move_cd > 1
            and counts[SCOUT] < 2
            and own_support_count < 2
            and factory_gap > 10
            and factory_energy >= 900
            and (counts[MINER] >= 1 or own_mines)
        ):
            record(factory_uid, "BUILD_SCOUT", factory_col, factory_row)
            counts[SCOUT] += 1
        elif f_move_cd > 1:
            ...
            if factory_uid in actions:
                ...
                route_key = (player, factory_uid)
                route_streaks = _memory.setdefault("terminal_route_block_streaks", {})
                route_main_step = bfs_jump(
                ...
                if route_main_step and not safe_factory_action(route_main_step):
                    route_main_step = None
                step = route_main_step

                route_closer_step = None
                if not step:
                    closer_goals = [(tc, min(north, factory_row + 5)) for tc in range(width)]
                    route_closer_step = bfs_first_step((factory_col, factory_row), closer_goals, depth=10, avoid_occupied=False)
                    if route_closer_step and not safe_factory_action(route_closer_step):
                        route_closer_step = None
                    step = route_closer_step

                route_blocked_now = route_main_step is None and route_closer_step is None
                prev_streak, prev_turn = route_streaks.get(route_key, (0, -99))
                if route_blocked_now and turn - prev_turn <= 4:
                    route_streak = prev_streak + 1
                elif route_blocked_now:
                    route_streak = 1
                else:
                    route_streak = 0
                route_streaks[route_key] = (route_streak, turn)
                ...
                weak_terminal_step = step in {
                    None, "IDLE", "SOUTH", "JUMP_SOUTH"
                } or (factory_gap <= 1 and step in {"EAST", "WEST", "JUMP_EAST", "JUMP_WEST"})
                true_final_kick = turn >= 400 or (south > 0 and factory_gap <= 1)
                if (
                    true_final_kick
                    and factory_gap <= 2
                    and route_streak >= 2
                    and weak_terminal_step
                ):
                    if (
                        f_jump_cd <= 0
                        and can_jump(factory_col, factory_row, "NORTH")
                        and safe_factory_action("JUMP_NORTH")
                        and (factory_col, factory_row + 2) not in my_positions
                    ):
                        step = "JUMP_NORTH"
                    elif (
                        f_move_cd <= 1
                        and can_move(factory_col, factory_row, "NORTH")
                        and safe_factory_action("NORTH")
                        and (factory_col, factory_row + 1) not in my_positions
                    ):
                        step = "NORTH"
