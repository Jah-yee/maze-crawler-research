...
"""
v62_2ply_forward: 2-ply forward-looking agent (Option γ).

Hybrid of v57 (v37 + Knob1 fix) + v56 cost function + 2-ply evaluation.

Key insight: v37's bfs_jump finds the FIRST path (BFS order), not the BEST.
v56's cost function scores positions but only looks 1-ply ahead.
2-ply: for each legal factory action, simulate it, then run BFS depth=8
from the resulting position, sum costs along the BFS path, pick the
root action with the lowest aggregate (root_cost + path_cost).
"""
import os
...
W_NORTH = float(os.environ.get("V62_W_NORTH", "5.0"))
W_COLLISION = float(os.environ.get("V62_W_COLLISION", "100"))
...
BFS_2PLY_DEPTH = int(os.environ.get("V62_2PLY_DEPTH", "8"))
