"""
v112: v67 + minimal terminal factory safety commit.

Keeps v67 hash/path perturbation and economy unchanged. Adds only a final
factory-action safety commit for terminal-risk states: if the factory would
idle/spend/go south while a safe north-progress action is available, prefer
JUMP_NORTH/NORTH. No support action overwrite.
"""
import hashlib
...
def _agent_inner(obs, config):
    ...
    # Minimal terminal safety commit: preserve route progress when the factory
    # is close to scroll pressure and the chosen action is terminally weak.
    f_action = actions.get(factory_uid, "IDLE")
    terminal_risk = turn >= 400 or factory_gap <= 4
    weak_terminal_action = (
        f_action in {"IDLE", "SOUTH", "JUMP_SOUTH"}
        or f_action.startswith("BUILD_")
        or f_action.startswith("TRANSFER_")
    )
    if terminal_risk and weak_terminal_action:
        replacement = None
        if (
            f_jump_cd <= 0
            ...
            replacement = "NORTH"
        if replacement:
            actions[factory_uid] = replacement
