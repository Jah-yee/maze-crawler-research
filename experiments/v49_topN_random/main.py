"""
v49 Maze Crawler Agent: v48 + ~30% top-2 randomization (D06 / W2 anti-IL).

Same as v48 but bumps the hash perturbation rate from ~5% to ~30%. With the
BFS being the dominant tie-break engine, swapping the lateral order is
approximately equivalent to "30% probability pick the second-best path"
when the two paths only differ in their first lateral step (which is the
common tie case for BFS in an open corridor).

Safety:
- Still gated by factory_gap > 6 (one row stricter than v48) so emergency
  and near-emergency turns stay deterministic.
- All other v37 invariants (north_only_jump, BUILD gates, etc.) untouched.

This is intentionally a small generalization of v48: we keep the same
mechanism but raise frequency to W2's recommended 30%.
"""
import hashlib
from collections import deque

_HASH_SALT = b"\x9a\x7d\xc4\x33\x88\x21\x55\x77"
_PERTURB_THRESHOLD = 77   # 77/256 ≈ 30.1% of turns swap lateral order
...
# p0 explores EAST before WEST; p1 explores WEST before EAST. 0% randomness.
...
def _agent_inner(obs, config):
    ...
    # Anti-mirror tie-break direction sets (D01/D02 + D03):
    # base = player-asymmetric DIRS; the final BFSDIRS / LATERAL etc. are
    # assigned later (after factory_gap is known) so the ~5% hash perturbation
    # can skip emergency turns.
    BFSDIRS_BASE = DIRS_P0 if player == 0 else DIRS_P1
    LATERAL_BASE = ("EAST", "WEST") if player == 0 else ("WEST", "EAST")
    # Provisional bindings so BFS function bodies can reference these names
    # if (defensively) called before the post-factory_gap re-binding. They
    # are always overwritten before any BFS call in normal flow.
    BFSDIRS = BFSDIRS_BASE
    LATERAL = LATERAL_BASE
    NEW_FALLBACK = ("NORTH",) + LATERAL
    DESPERATION_JUMP = LATERAL + ("SOUTH",)
    ...
    def bfs_jump(start, goals, jump_cd, depth=20, north_only_jump=False):
        ...
        jump_dirs = ("NORTH",) if north_only_jump else BFSDIRS
        while q:
    ...
    # D03: ~5% turns swap the lateral order for this turn's tie-break.
    # Gated to safe turns (gap > 4) so emergency / final-kick logic uses the
    # stable player-asymmetric ordering. The salt + 4-byte key makes the
    # schedule deterministic per (turn, factory_col, factory_row, player)
    # but unpredictable to imitators without the salt.
    if factory_gap > 6:
        _h = hashlib.sha256(
            _HASH_SALT
            + bytes([turn % 256, factory_col % 256, factory_row % 256, player & 0xFF])
        ).digest()[0]
        if _h < _PERTURB_THRESHOLD:
            LATERAL = LATERAL_BASE[::-1]
            BFSDIRS = ("NORTH",) + LATERAL + ("SOUTH",)
            NEW_FALLBACK = ("NORTH",) + LATERAL
            DESPERATION_JUMP = LATERAL + ("SOUTH",)
