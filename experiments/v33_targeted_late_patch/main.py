"""
v33 Maze Crawler Agent: v24 with narrow late-game scroll/contact patches.
Key ideas: jump-preferred BFS, mirror vision, optimistic fog, emergency escape,
an early no-scout window for bunterrrrr-style mine conversion, and two
late-game fixes that stay out of the early-game race.
"""
...
SCOUT_DELAY_STEP = 24
LATE_SCROLL_STEP = 430
LATE_SCROLL_SOUTH = 90
LATE_SCROLL_GAP = 8
...
def _agent_inner(obs, config):
    ...
    late_scroll_mode = turn >= LATE_SCROLL_STEP or south >= LATE_SCROLL_SOUTH
    urgent_scroll_gap = 2
    if late_scroll_mode:
        urgent_scroll_gap = LATE_SCROLL_GAP
    ...
    def escape_adjacent_enemy_factory():
        if f_move_cd > 1 or safe_factory_action("IDLE"):
            return None
        for enemy in enemy_factories:
            ec, er = enemy[1], enemy[2]
            emove_cd = enemy[5] if len(enemy) > 5 else 0
            if abs(ec - factory_col) + abs(er - factory_row) != 1 or emove_cd > 1:
                continue
            enemy_support_ok = visible_enemy_support_energy + 20 >= own_support_energy
            enemy_count_ok = visible_enemy_support_count > own_support_count
            if not (enemy_support_ok or enemy_count_ok):
                continue
            options = []
            for d in ("NORTH", "EAST", "WEST", "SOUTH"):
                if not can_move(factory_col, factory_row, d) or not safe_factory_action(d):
                    continue
                dc, dr = OFFSETS[d]
                dest = (factory_col + dc, factory_row + dr)
                dist = abs(dest[0] - ec) + abs(dest[1] - er)
                if dist <= 1:
                    continue
                north_gain = dest[1] - factory_row
                options.append((north_gain, dist, d))
            if options:
    ...
    # Emergency: widen only in true late-game scroll pressure.
    if factory_gap <= urgent_scroll_gap and south > 0:
        ...
        elif f_move_cd <= 1 and can_move(factory_col, factory_row, "NORTH") and safe_factory_action("NORTH"):
            nc, nr = factory_col, factory_row + 1
            if (nc, nr) not in my_positions or my_positions.get((nc, nr)) == factory_uid:
                record(factory_uid, "NORTH", factory_col, factory_row)
        elif f_jump_cd <= 0:
            for d in ("EAST", "WEST"):
                act = f"JUMP_{d}"
    ...
    if factory_uid not in actions:
        ...
        if (
            ...
            and factory_gap > 5
            and counts[MINER] < 1
            and not (late_scroll_mode and factory_gap <= urgent_scroll_gap + 2)
        ):
            for d in ("NORTH",):
                dc, dr = OFFSETS[d]
        ...
        if mine_build_action:
            ...
            escape_step = escape_adjacent_enemy_factory()
            if escape_step:
                record(factory_uid, escape_step, factory_col, factory_row)
            if (
                factory_uid not in actions
                and (factory_col, factory_row) in own_mines
                and factory_gap > 10
                and not late_scroll_mode
                and safe_factory_action("IDLE")
            ):
                record(factory_uid, "IDLE", factory_col, factory_row)
            ...
            if factory_uid in actions:
                ...
                if step and step != "IDLE":
                    ...
                    if spawn_ok and counts[WORKER] < 1 and factory_energy >= 200 and factory_gap <= urgent_scroll_gap + 2:
