"""
v57_knob1_fix: v37 + P01 enemy-current-cell fix only.

Built on v37. One-line behavioral change: the enemy factory's CURRENT cell
is now unconditionally treated as an unsafe destination (not only when
collision_tiebreak_bad is True). v40/v37/v51 all had the same bug:
walking NORTH into a stationary enemy factory because the tiebreak heuristic
said "not at risk" (replay 78598494). Ramming an enemy factory destroys
our own factory -> at best a draw, so its cell should never be a destination.

This is the minimal correct fix extracted from v42's P01 + Final Kick bundle.
"""
...
def _agent_inner(obs, config):
    ...
    # ========== P01 FIX: enemy factory's current cell is ALWAYS unsafe ==========
    # v37/v40/v51 had this inside `if collision_tiebreak_bad:` — when the
    # heuristic said "not at risk", the enemy's own cell was never blocked,
    # causing the factory to walk into a stationary enemy (replay 78598494).
    # Ramming an enemy = mutual destruction = at best a draw.
