"""
v46d = v37 + emergency expansion at gap==3.

When factory_gap == 3 AND f_jump_cd <= 0 AND JUMP_NORTH is available and safe,
preemptively JUMP_NORTH (extends gap to 5). Trades a fresh 20-turn jump
cooldown for a 2-row buffer; safer than descending to gap=2 where a mvcd>0
can pin us into scroll-death.
"""
...
def _agent_inner(obs, config):
    ...
    # D4: preemptive escape at gap==3 (NORTH jump). Run BEFORE the gap<=2
    # block so it short-circuits when a clean JUMP_NORTH exists.
    if (
        factory_gap == 3
        and south > 0
        and f_jump_cd <= 0
