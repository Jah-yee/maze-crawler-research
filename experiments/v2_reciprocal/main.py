...
def _agent_inner(obs, config):
    ...
    def remember_cell(c, r, w, exact=False):
        if not (0 <= c < width):
            return
        if exact or (c, r) not in walls_mem:
            walls_mem[(c, r)] = w
        else:
            walls_mem[(c, r)] |= w
        for d, bit in WALL_BITS.items():
            if not (w & bit):
                continue
            dc, dr = OFFSETS[d]
            nc, nr = c + dc, r + dr
            if 0 <= nc < width:
                walls_mem[(nc, nr)] = walls_mem.get((nc, nr), 0) | WALL_BITS[OPPOSITE[d]]
    ...
    for i, w in enumerate(obs.walls):
        ...
        r = south + i // width
        c = i % width
        remember_cell(c, r, w, exact=True)
        # Mirror: col' = (width - 1) - col, swap E<->W bits
        mc = (width - 1) - c
        mw = MIRROR_WALL[w]
        if (mc, r) not in walls_mem:
            remember_cell(mc, r, mw, exact=True)
    ...
    danger_gap = 2 + (1 if step_no >= 300 else 0) + (1 if step_no >= 425 else 0)
    ...
    # Emergency: rows close to the south bound become more dangerous late game.
    if factory_gap <= danger_gap and south > 0:
    ...
    if factory_uid not in actions:
        ...
        if spawn_ok and counts[SCOUT] < 1 and factory_energy >= 50:
            ...
            if not step and factory_gap <= danger_gap + 1:
                if can_move(factory_col, factory_row, "SOUTH"):
                    step = "SOUTH"

            # Fallback 4: desperation jump any direction
            if not step and f_jump_cd <= 0 and factory_gap <= danger_gap + 1:
            ...
            if step and step != "IDLE":
                ...
                if spawn_ok and counts[WORKER] < 1 and factory_energy >= 200 and factory_gap <= danger_gap + 2:
