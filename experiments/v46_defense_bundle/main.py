"""
v46_defense_bundle = v37 + 4 defensive changes targeting boundary_scroll deaths.

Base: experiments/v37_late_save_jump_no_scout/main.py (unchanged ECON/BUILD/DIRS).
All changes are clamped to the factory survival path; SCOUT/MINER/WORKER blocks
are untouched (those are owned by the economy / anti-mirror workers).

D1 ENERGY HARD FLOOR (W2/W10): if factory_energy < max((500-turn)*1.5, 500),
   disable BUILD_MINER / BUILD_SCOUT / BUILD_WORKER. Targets "energy=0 forced
   IDLE -> scrolled" deaths.

D2 LATE-GAME BOUNDARY (W12): if turn >= 400 AND factory_gap <= 10, ban SOUTH,
   JUMP_SOUTH, JUMP_EAST, JUMP_WEST and all BUILD_*. Lateral *walks*
   (EAST/WEST) are still allowed, unlike v42b which over-fit by banning them.

D3 JUMP CD PRESERVATION: widen north_only_jump in BFS from gap<=8 to gap<=12;
   tighten desperation lateral jump fallback from gap<=3 to gap<=2. Together
   these stop the v30 step-481 pattern (lateral jump locks jcd=20 then die).

D4 EMERGENCY EXPANSION: at gap == 3 with jump ready AND JUMP_NORTH available,
   preemptively JUMP_NORTH (gap becomes 5) instead of waiting for gap == 2.
   Eats 20 turn cooldown but trades it for a 2-row buffer; safer than
   descending to gap=2 where mvcd>0 can pin us.
"""
...
SCOUT_DELAY_STEP = 24
LATE_BOUNDARY_STEP = 400
LATE_BOUNDARY_GAP = 10
...
def _agent_inner(obs, config):
    ...
    # ===== D1: energy hard floor =====
    energy_floor = max(int((500 - turn) * 1.5), 500)
    low_energy_lock = factory_energy < energy_floor

    # ===== D2: late-game boundary =====
    late_boundary = turn >= LATE_BOUNDARY_STEP and factory_gap <= LATE_BOUNDARY_GAP

    # Combined "no build" gate
    no_build = low_energy_lock or late_boundary
    ...
    def safe_factory_action(act):
        ...
        if act.startswith("JUMP_") and dest in enemy_factory_jump_threats:
            return False
        # D2: in late-game boundary mode, kill anything that goes south or
        # burns the jump cd on a lateral. Lateral *walks* still pass.
        if late_boundary:
            if act in ("SOUTH", "JUMP_SOUTH", "JUMP_EAST", "JUMP_WEST"):
                return False
        return True

    # ========== EMERGENCY ESCALATIONS ==========
    # D4: at gap==3 with jump ready, preemptive JUMP_NORTH (extends gap to 5).
    # Safer than waiting for gap==2 where mvcd may pin us.
    if (
        factory_gap == 3
        and south > 0
        and f_jump_cd <= 0
        and can_jump(factory_col, factory_row, "NORTH")
        and safe_factory_action("JUMP_NORTH")
    ):
    ...
    if factory_uid not in actions and factory_gap <= 2 and south > 0:
        if f_jump_cd <= 0 and can_jump(factory_col, factory_row, "NORTH") and safe_factory_action("JUMP_NORTH"):
            ...
            for d in ("EAST", "WEST"):
                act = f"JUMP_{d}"
                # Emergency at gap<=2 ignores D2 lateral-jump ban: we are about
                # to die anyway, a lateral jump that survives 1 more turn is
                # strictly better than scroll-death this turn.
                dest = action_dest(factory_col, factory_row, act)
                blocked = (
                    (enemy_factory_threats and dest in enemy_factory_threats)
                    or dest in enemy_factory_jump_threats
                )
                if can_jump(factory_col, factory_row, d) and not blocked:
                    dc, dr = OFFSETS[d]
    ...
    if factory_uid not in actions:
        ...
        if mine_build_action:
            ...
            # D2: in late_boundary mode don't camp on own mine — race north.
            if (not late_boundary) and (factory_col, factory_row) in own_mines and factory_gap > 10:
                record(factory_uid, "IDLE", factory_col, factory_row)
            ...
            if factory_uid in actions:
                ...
                # D3: widen north_only_jump from gap<=8 to gap<=12 to keep
                # jump_cd available across a larger danger band.
                ...
                step = bfs_jump(
                    ...
                    f_jump_cd,
                    depth=20,
                    north_only_jump=factory_gap <= 12,
                )
                ...
                # Fallback 3: SOUTH walk to escape dead-end. D2 forbids this in
                # late_boundary; safe_factory_action already rejects SOUTH then.
                if not step and factory_gap <= 3:
                    if can_move(factory_col, factory_row, "SOUTH") and safe_factory_action("SOUTH"):
                        step = "SOUTH"

                # D3: desperation lateral jump tightened gap<=3 -> gap<=2 so we
                # almost never burn jump_cd on a lateral. The top emergency
                # block already covers gap<=2 with JUMP_NORTH preference.
                if not step and f_jump_cd <= 0 and factory_gap <= 2:
                    for d in ("EAST", "WEST", "SOUTH"):
                        act = f"JUMP_{d}"
                        # bypass late_boundary lateral-jump ban here: this is
                        # the same "about to die" exception as the emergency block.
                        dest = action_dest(factory_col, factory_row, act)
                        blocked = (
                            (enemy_factory_threats and dest in enemy_factory_threats)
                            or dest in enemy_factory_jump_threats
                ...
                if step and step != "IDLE":
                    ...
                    # D1/D2: same energy/late gates for the last-resort worker
                    if (
                        (not no_build)
                        and spawn_ok
                        and counts[WORKER] < 1
