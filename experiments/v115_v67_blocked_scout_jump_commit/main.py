"""
v115: v67 + blocked-scout NORTH jump ownership commit.

Single-variable candidate: when the factory planned NORTH under route pressure
but the north cell is held by an IDLE friendly scout, preserve factory route
ownership with a safe JUMP_NORTH if its landing is clear. The scout-only gate
matches all observed active-v67 blocker events. Support behavior, economy, and
all other v67 decisions stay unchanged.
"""
import hashlib
...
def _agent_inner(obs, config):
    ...
    # Preserve factory route ownership without rewriting support actions.
    f_action = actions.get(factory_uid, "IDLE")
    north_pos = (factory_col, factory_row + 1)
    blocker_uid = my_positions.get(north_pos)
    if (
        f_action == "NORTH"
        and factory_gap <= 8
        and blocker_uid
        and blocker_uid != factory_uid
        and my_robots[blocker_uid][0] == SCOUT
        and actions.get(blocker_uid, "IDLE") == "IDLE"
        and f_jump_cd <= 0
        ...
        actions[factory_uid] = "JUMP_NORTH"
