"""
v50 Utility Scoring Architecture PoC.

Behaviorally 1:1 equivalent to v37 (`v37_late_save_jump_no_scout`). The only
change vs v37 is structural: the factory decision is no longer a long elif
chain, it is a list of small `scorer` functions that each emit one
`(action, score)` proposal. The final factory action is the argmax over all
scorers. Helper functions, scout/worker/miner planning and the BFS code are
copied from v37 verbatim, because the goal of this PoC is to validate the
container, not to introduce a new strategy.

Why this exists (philosophy, W10 - The Surviving Compounder):
    "Survive first, compound second, sprint last."

Every future patch (better late-game switch, TRANSFER from miner, side-mine
gate, anti-mirror perturbation, etc.) becomes one new `score_*` function in
SCORERS. The priority-band scheme below makes the equivalence with v37's
elif ordering explicit and easy to audit.
"""
