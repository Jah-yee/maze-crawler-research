...
def _agent_inner(obs, config):
    ...
    def bfs_jump(start, goals, jump_cd, depth=20):
        """
        Weighted jump-aware search.
        It keeps the public baseline's jump preference, but also nudges
        paths toward net north progress and away from SOUTH detours.
        """
        if not goals:
            return None
        goal_set = set(goals)
        start_row = start[1]
        q = [(0, 0, 0, start, None, min(jump_cd, 20))]
        best = {(start[0], start[1], jump_cd <= 0): 0}
        counter = 0
        while q:
            _, dist, _, (c, r), first_d, jcd = heappop(q)
            if (c, r) in goal_set and dist > 0:
                return first_d
            if dist >= depth:
                continue
            ...
            if jcd <= 0:
                for d in DIRS:
                    ...
                    dc, dr = OFFSETS[d]
                    nc, nr = c + 2 * dc, r + 2 * dr
                    candidates.append((f"JUMP_{d}", nc, nr, 20))

            for d in DIRS:
                if not can_move(c, r, d):
                    continue
                nc, nr = c + OFFSETS[d][0], r + OFFSETS[d][1]
                candidates.append((d, nc, nr, max(0, jcd - 1)))

            for act, nc, nr, njcd in candidates:
                key = (nc, nr, njcd <= 0)
                north_gain = nr - start_row
                south_penalty = 12 if nr < r else 0
                lateral_penalty = 2 if nr == r else 0
                jump_bonus = 5 if act == "JUMP_NORTH" else 0
                priority = (dist + 1) * 10 - north_gain * 3 + south_penalty + lateral_penalty - jump_bonus
                if best.get(key, 10**9) <= priority:
                    continue
                best[key] = priority
                counter += 1
                heappush(q, (priority, dist + 1, counter, (nc, nr), first_d or act, njcd))
