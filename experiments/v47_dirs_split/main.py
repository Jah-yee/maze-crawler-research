"""
v47 Maze Crawler Agent: v37 + player-asymmetric DIRS for self-play anti-mirror (D01/D02).

Only change vs v37: every tie-break iteration order that depends on EAST/WEST
is split by `obs.player`. Player 0 keeps the original DIRS (EAST before WEST),
player 1 swaps to (WEST before EAST). This is 0% randomness, fully
deterministic per player, but breaks the perfect mirror symmetry that makes
v37 vs v37 produce many draws / mutual-kills.

No other behavior change: BUILD_MINER / TRANSFER / boundary defense are
untouched (those belong to economy / defense workers).
"""
...
def _agent_inner(obs, config):
    ...
    # Anti-mirror tie-break direction sets (D01/D02): all subsequent loops
    # that previously iterated DIRS or ("NORTH","EAST","WEST") / ("EAST","WEST")
    # now go through these to split player 0 and player 1 deterministically.
    BFSDIRS = DIRS_P0 if player == 0 else DIRS_P1
    LATERAL = ("EAST", "WEST") if player == 0 else ("WEST", "EAST")
