"""
v43 Maze Crawler Agent: v37 + one late worker buffer (W8 #1 ROI).
...
def _agent_inner(obs, config):
    ...
    if factory_uid not in actions:
        ...
        if mine_build_action:
            ...
            # W8 #1 ROI: one late worker as an end-game energy / tiebreak
            # buffer. We are the only top-bracket agent that builds 0 workers.
