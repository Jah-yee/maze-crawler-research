"""
v35 Maze Crawler Agent: v24 with corrected emergency escape.
Key ideas: jump-preferred BFS, mirror vision, optimistic fog, emergency escape,
and an early no-scout window for bunterrrrr-style mine conversion.
v35 fix: emergency block now (a) requires both jump+move cooldowns clear for
JUMP_NORTH, (b) tries a regular NORTH walk when JUMP is on cooldown, and (c)
when factory_gap <= 1 overrides enemy-factory collision avoidance to prefer
survival over a clean tiebreak.
"""
...
def _agent_inner(obs, config):
    ...
    # Emergency: <= 2 rows from south bound = imminent death.
    # The engine requires BOTH move_cooldown and jump_cooldown <= 0 for JUMP
    # (crawl.py phase-4); if we ignore move_cd here we can issue an action that
    # silently no-ops and the factory dies on the next scroll. When the gap is
    # critical (<= 1) we also override safe_factory_action so a clean
    # tiebreak collision is preferred over a guaranteed boundary death.
    can_jump_now = f_move_cd <= 1 and f_jump_cd <= 0
    if factory_gap <= 2 and south > 0:
        critical = factory_gap <= 1
        def safe_or_critical(act):
            if safe_factory_action(act):
                return True
            return critical
        ...
        if (
            ...
            and safe_or_critical("JUMP_NORTH")
        ):
        ...
        if (
            ...
            and f_move_cd <= 1
            and can_move(factory_col, factory_row, "NORTH")
            and safe_or_critical("NORTH")
        ):
            nc, nr = factory_col, factory_row + 1
            if (nc, nr) not in my_positions or my_positions.get((nc, nr)) == factory_uid:
                record(factory_uid, "NORTH", factory_col, factory_row)

        if factory_uid not in actions and can_jump_now:
            for d in ("EAST", "WEST"):
                act = f"JUMP_{d}"
                if can_jump(factory_col, factory_row, d) and safe_or_critical(act):
                    dc, dr = OFFSETS[d]
        ...
        if factory_uid not in actions and critical and f_move_cd <= 1:
            for d in ("EAST", "WEST"):
                if can_move(factory_col, factory_row, d) and safe_or_critical(d):
                    dc, dr = OFFSETS[d]
    ...
    if factory_uid not in actions:
        ...
        if f_build_cd <= 1 and factory_energy >= 650 and factory_gap > 5 and counts[MINER] < 1:
            for d in ("NORTH",):
                dc, dr = OFFSETS[d]
        ...
        if mine_build_action:
            ...
            counts[MINER] += 1
        elif turn >= SCOUT_DELAY_STEP and spawn_ok and counts[SCOUT] < 1 and factory_energy >= 50:
