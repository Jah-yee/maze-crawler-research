"""
v46c = v37 + jump-cd preservation only.

Two micro-changes to keep the 20-turn jump cooldown ready for NORTH emergencies:
1. BFS north_only_jump band widened from gap<=8 to gap<=12.
2. Fallback-4 desperation lateral jump tightened from gap<=3 to gap<=2.

The v30 step-481 pattern (a lateral jump at gap=4 locks jcd=20, then we die
when gap drops to 2 with jcd still cooling) needs the BFS guard to extend
further plus the fallback to fire only at true gap<=2 emergencies.
"""
