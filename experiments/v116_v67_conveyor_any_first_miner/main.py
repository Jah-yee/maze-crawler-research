"""
v116: v67 + CONVEYOR non-NORTH first-miner placement.

Keep v67's original NORTH miner rule unchanged. When the first miner is still
missing during the stable CONVEYOR phase, also permit an adjacent EAST, WEST,
or SOUTH mining node under the same build-cooldown, energy, and gap gates.
"""
import hashlib
...
def _agent_inner(obs, config):
    ...
    if factory_uid not in actions:
        ...
        if f_build_cd <= 1 and factory_energy >= 650 and factory_gap > 6 and counts[MINER] < 1:
            conveyor_phase = 30 <= turn < 200 and factory_gap > 8 and factory_energy >= 300
            miner_dirs = ("NORTH", "EAST", "WEST", "SOUTH") if conveyor_phase else ("NORTH",)
            for d in miner_dirs:
                dc, dr = OFFSETS[d]
