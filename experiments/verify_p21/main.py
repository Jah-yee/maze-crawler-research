"""
verify_p21: v37 + W9/W8 P21 single-branch BUILD_WORKER_NORTH probe.

Minimal diff from v37: between the SCOUT build branch and the f_move_cd > 1
branch we add a single elif that spawns one worker when factory is healthy
(gap > 6, energy >= 250). Nothing else is changed.
...
def _agent_inner(obs, config):
    ...
    if factory_uid not in actions:
        ...
        if mine_build_action:
            ...
            spawn_ok
            and counts[WORKER] < 1
            and factory_energy >= 250
            and factory_gap > 6
        ):
            record(factory_uid, "BUILD_WORKER_NORTH", factory_col, factory_row)
            counts[WORKER] += 1
        elif f_move_cd > 1:
