"""
v26 Maze Crawler Agent: v15 north mine economy with a narrow lateral jump guard.
Key ideas: jump-preferred BFS, mirror vision, optimistic fog, emergency escape,
and avoiding lateral factory jumps into nearby enemy factory move threats.
"""
...
def _agent_inner(obs, config):
    ...
    enemy_factory_lateral_jump_threats = set()
    ...
    if factory_gap > 3:
        for enemy in enemy_factories:
            ...
            if abs(ec - factory_col) + abs(er - factory_row) > 4:
                continue
            emove_cd = enemy[5] if len(enemy) > 5 else 0
            enemy_factory_lateral_jump_threats.add((ec, er))
            if emove_cd <= 1:
                for d in DIRS:
                    if can_move(ec, er, d):
                        enemy_factory_lateral_jump_threats.add(action_dest(ec, er, d))
    ...
    def safe_factory_action(act):
        ...
        if enemy_factory_threats and dest in enemy_factory_threats:
            return False
        if act in ("JUMP_EAST", "JUMP_WEST", "JUMP_SOUTH") and dest in enemy_factory_lateral_jump_threats:
            return False
        return True
    ...
    if factory_uid not in actions:
        ...
        if mine_build_action:
            ...
            counts[MINER] += 1
        elif spawn_ok and counts[SCOUT] < 1 and factory_energy >= 50:
