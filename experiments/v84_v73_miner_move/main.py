...
def _agent_inner(obs, config):
    ...
    if factory_uid not in actions:
        ...
        if f_build_cd <= 1 and factory_energy >= 350 and factory_gap > 3 and counts[MINER] < 1:
            for d in ("NORTH",):
                dc, dr = OFFSETS[d]
    ...
    # ========== MINER PLANNING (early miner with TRANSFER support) ==========
    # Priority: TRANSFER to struggling factory > TRANSFORM when surplus > pathfind
    miners = [(uid, d) for uid, d in my_robots.items() if d[0] == MINER]
    for uid, data in miners:
        col, row = data[1], data[2]
        energy = data[3]
        move_cd = data[5] if len(data) > 5 else 0
        adj = abs(col - factory_col) + abs(row - factory_row) == 1
        on_node = (col, row) in mining_nodes
        if adj and factory_energy < 500 and energy >= 50:
            if row < factory_row:
                record(uid, "TRANSFER_NORTH", col, row)
            elif row > factory_row:
                record(uid, "TRANSFER_SOUTH", col, row)
            elif col < factory_col:
                record(uid, "TRANSFER_EAST", col, row)
            else:
                record(uid, "TRANSFER_WEST", col, row)
        elif col == factory_col and row == factory_row + 1 and move_cd <= 1:
            for _d in ("EAST", "WEST"):
                _dc, _dr = OFFSETS[_d]
                _nc, _nr = col + _dc, row + _dr
                if can_move(col, row, _d) and (_nc, _nr) not in my_positions:
                    record(uid, _d, col, row)
                    break
            else:
                if on_node and energy >= 300:
                    ...
                    record(uid, "IDLE", col, row)
        elif on_node and energy >= 300:
            record(uid, "TRANSFORM", col, row)
        elif on_node and energy >= 100:
            record(uid, "TRANSFORM", col, row)
        elif move_cd <= 1:
            goal_nodes = [n for n in mining_nodes if abs(n[0] - col) + abs(n[1] - row) <= 6]
            if goal_nodes:
                best = min(goal_nodes, key=lambda n: abs(n[0] - col) + abs(n[1] - row))
                step = bfs_first_step((col, row), [best], depth=8, avoid_occupied=False)
                if step and step != "IDLE":
                    record(uid, step, col, row)
                else:
                    record(uid, "IDLE", col, row)
