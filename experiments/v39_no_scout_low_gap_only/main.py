"""
v39 Maze Crawler Agent: v24 + only the build-gating fix from v37.

Drops v37's jump expansion restriction so we can isolate whether the
+8/100 paired-seed signal vs v24 came from the build gate or the jump
restriction. Keeps a single-knob change vs v24:

- BUILD_SCOUT requires factory_gap > 4 (v24 had no gap check here).
- BUILD_MINER requires factory_gap > 6 (v24 used > 5).
"""
