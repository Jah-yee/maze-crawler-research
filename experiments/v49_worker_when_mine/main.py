"""
v49: v48 + late worker when mine active (no rebuild after first scout dies) v24 + two narrow fixes for boundary_scroll deaths.
    ...
    if factory_uid not in actions:
        ...
        if f_build_cd <= 1 and counts[MINER] < 1:
            ...
            if mpos in mining_nodes and mpos not in my_positions and can_move(factory_col, factory_row, "NORTH"):
                if turn <= 25 and factory_energy >= 400:
                    # Early window: mine node directly north and cheap enough
                    mine_build_action = "BUILD_MINER_NORTH"
                elif factory_energy >= 650 and factory_gap > 6:
                    # Normal window: standard gates
                    mine_build_action = "BUILD_MINER_NORTH"
        ...
        if mine_build_action:
            ...
            and spawn_ok
            and counts[SCOUT] < 1
            and _memory.get("scouts_built", 0) < 1  # total cap: 1 scout per episode
            ...
            and counts[WORKER] == 0
            and len(own_mines) >= 1  # only build worker when mine income is running
        ):
            # Late worker: only when mine is active. Worker adds energy to
            # tiebreak pool (but only worth it if mine offsets the factory cost).
