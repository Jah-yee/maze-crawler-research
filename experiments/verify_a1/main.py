"""
verify_a1: v37 + W6 A1 factory-crush probe.

Minimal diff from v37: in the main factory pathfinding branch (after own_mines
checks, before bfs_jump), if factory_gap > 4 and there is an adjacent enemy
support unit (non-factory) the factory can reach by a single walk that is also
safe_factory_action, prefer that ram move. Everything else unchanged.
    ...
    if factory_uid not in actions:
        ...
        if mine_build_action:
            ...
            if factory_uid in actions:
                ...
                # A1 factory-crush probe: if a non-factory enemy is in an
                # adjacent reachable cell and we are healthy, ram it.
                enemy_support_positions = {
                    (d[1], d[2]) for d in enemy_robots.values() if d[0] != FACTORY
                }
                if factory_gap > 4 and enemy_support_positions:
                    for d in DIRS:
                        dc, dr = OFFSETS[d]
                        target = (factory_col + dc, factory_row + dr)
                        if target in enemy_support_positions and can_move(factory_col, factory_row, d) and safe_factory_action(d):
                            record(factory_uid, d, factory_col, factory_row)
                            break
            if factory_uid in actions:
                pass
